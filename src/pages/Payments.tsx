  ...
  const handleFeatureClick = async (feature: string, payment?: MappedPayment) => {
    ...
    if (feature === 'download-receipt' && payment) {
      ...
      try {
        ...
        doc.setFontSize(8);
        doc.setTextColor(180, 210, 235);
        doc.text(`#${payment.id.slice(0, 12).toUpperCase()}`, pageW - margin, 19, { align: 'right' });
        ...
        doc.text(`$${payment.amount}`, pageW / 2, 52, { align: 'center' });
        doc.setFontSize(8);
        doc.setFont('helvetica', 'normal');
        ...
        const rows: [string, string][] = [
          ['Date',       payment.date ? format(new Date(payment.date), 'MMMM d, yyyy') : '—'],
          ['Game',       gameLabel],
        ...
        // Status badge
        const statusColor = payment.status === 'paid' ? GREEN : [202, 138, 4] as [number, number, number];
        doc.setFillColor(...statusColor);
        doc.roundedRect(margin, y + 4, 28, 7, 2, 2, 'F');
        doc.setTextColor(...WHITE);
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(8);
        doc.text(payment.status.toUpperCase(), margin + 14, y + 9, { align: 'center' });
        // Footer
        ...
        doc.save(`iWhistle_Receipt_${payment.id.slice(0, 8).toUpperCase()}.pdf`);
        toast({ title: 'Receipt downloaded', description: 'PDF receipt saved.' });
      } catch {
